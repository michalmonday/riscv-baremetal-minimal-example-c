...
static char * correct_passwords[] = {"1234", "0123456789012"}; 
...
bool is_password_correct(char *password) {
    static char * correct_passwords[] = {"1234", "0123456789012"};
    for (int i = 0; i < sizeof(correct_passwords) / sizeof(correct_passwords[0]); i++) {
        if (!strcmp(password, correct_passwords[i])) {
            return true;
        }
    }
    return false;
