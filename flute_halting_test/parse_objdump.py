...
def parse_objdump(f_name, ignore_prefix=None, ignore_substring=None):
    ...
    for name, body in funcs_sections:
        if ignore_prefix and name.startswith(ignore_prefix):
            continue
        if ignore_substring and ignore_substring in name:
            continue
        funcs[name] = {}
        ...
        for i, line in enumerate(lines):
            ...
            if values is None:
                print("Ignoring line while parsing objdump:", line.strip())
                continue
            pc, instr, instr_name, is_function, func_destination_name, is_branch, branch_destination = values
...
if __name__ == '__main__':
    ...
    parser.add_argument(
            ...
            metavar='',
            type=str,
            help='Ignore functions with this prefix.'
            )
    parser.add_argument(
            '--ignore-substring',
            required=False,
            ...
            metavar='',
            type=str,
            help='Ignore functions containing substring.'
            )


    args = parser.parse_args()

    ranges = parse_objdump(args.file_name.name, args.ignore_prefix, args.ignore_substring)
